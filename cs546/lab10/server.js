...
// 8. Added for lab 10
let logData = require('./logData');
let uuid = require('node-uuid');
// Match "/api" "/api/" "/api/path" "/api/path/even/longer?querystring=true"
app.use("/api(/)?*", function( request, response, next) {
    console.log("####################");
    console.log("/api/* path accessed");
    console.log("####################");
    let reqId = uuid.v4();
    let reqPath = request.baseUrl;
    let reqMethod = request.method;
    let cookies = request.cookies;
    logData.createLog(reqId, reqPath, reqMethod, cookies);
    next();
});
// Added for lab 10
app.get("/cookies/addCookie", (req, res) => {
    let reqK = req.query.key;
    let reqV = req.query.value;
    if( (typeof reqK == 'string') && (typeof reqV == 'string') ){
        res.cookie(reqK, reqV);
        res.status(200).send("This succeeded");
    }else{
        res.status(500).send("Key or value recieved was not a string");
    }
});
